...
void MODEL_GetTopN(const void* tensorData, int tensorSize, tensor_type_t tensorType,
                   size_t numResults, float threshold, result_t* topResults)
{
  ...
  const float* floatPredictions = nullptr;
  const int8_t* intPredictions = nullptr;
  const uint8_t* uintPredictions = nullptr;

  if (tensorType == kTensorType_FLOAT32)
  {
      floatPredictions = reinterpret_cast<const float*>(tensorData);
  }
  else if (tensorType == kTensorType_INT8)
  {
      intPredictions = reinterpret_cast<const int8_t*>(tensorData);
  }
  else
  {
      uintPredictions = reinterpret_cast<const uint8_t*>(tensorData);
  }
  ...
  for (int i = 0; i < tensorSize; i++) {
    ...
    if (tensorType == kTensorType_FLOAT32) {
        value = floatPredictions[i];
    } else if (tensorType == kTensorType_UINT8) {
        value = uintPredictions[i] / 255.0;
    } else if (tensorType == kTensorType_INT8) {
        value = ((int)intPredictions[i] + 128) / 255.0;
    }
