...
int main(void)
{
    ...
    normalize(ECGbufferV, 256);

    tensor_dims_t inputDims = {1, 256, 1};  // 輸入張量的尺寸為 {3, {1, 256, 1}}; size = 3，表示這個張量有 3 個維度
    tensor_type_t inputType = kTensorType_INT8;  // 輸入類型為 INT8
    ...
    // 轉換 inputData 指標為 int8_t*，以確保數據格式正確
    int8_t* int8Data = reinterpret_cast<int8_t*>(inputData);

    // 將 ECGbufferN 資料複製到模型輸入
    memcpy(int8Data, ECGbufferV, sizeof(ECGbufferV));
